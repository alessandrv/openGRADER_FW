...
#define I2C_MSG_LAYER_STATE 0x03
...
typedef struct {
    ...
    uint8_t layer_mask; // Active layer bitmask (shared across devices)
...
typedef struct {
    uint8_t header;       // Always I2C_MSG_HEADER (0xAB)
    uint8_t msg_type;     // I2C_MSG_LAYER_STATE (0x03)
    uint8_t layer_mask;   // Active layer mask
    uint8_t default_layer;// Default layer index
    uint8_t reserved0;    // Reserved/padding
    uint8_t reserved1;    // Reserved/padding
    uint8_t reserved2;    // Reserved/padding
    uint8_t checksum;     // Simple checksum for data integrity
} __attribute__((packed)) i2c_layer_state_t;
...
{
    return msg->header + msg->msg_type + msg->row + msg->col + msg->pressed + msg->keycode + msg->layer_mask;
}
...
static inline uint8_t i2c_calc_layer_checksum(const i2c_layer_state_t *msg)
{
    return msg->header + msg->msg_type + msg->layer_mask + msg->default_layer;
}
...
static inline uint8_t i2c_validate_layer_message(const i2c_layer_state_t *msg)
{
    return (i2c_calc_layer_checksum(msg) == msg->checksum) ? 1 : 0;
}
